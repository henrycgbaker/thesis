...
from _3_prompt_processing import batch_tokenise_truncate  # Ensure this function is imported
...
def run_gen_inference(model, experiment_config, prompts, tokenizer, accelerator):
    ...
    Returns:
        A tuple (text_outputs, concatenated_input_ids, inference_results) where:
          - text_outputs is a list of generated output batches.
          - concatenated_input_ids is a tensor containing all input_ids across batches.
          - inference_results is a dictionary of computed inference metrics.
    """
    ...
    fixed_batch_size = experiment_config.batching_options.get("max_batch_size", 8)
    ...
    if use_adaptive:
        ...
        # adaptive_batching returns a list of batches (each a list of prompt strings)
        batches = adaptive_batching(prompts, tokenizer, adaptive_max_tokens, max_batch_size=fixed_batch_size)
        accelerator.print(f"Adaptive batching created {len(batches)} batches.")
    else:
        # Fixed batching: simply partition the prompts into fixed-size chunks.
        batches = [prompts[i:i+fixed_batch_size] for i in range(0, len(prompts), fixed_batch_size)]
        accelerator.print(f"Fixed batching created {len(batches)} batches.")
    
    # Process each batch: tokenise and run inference immediately.
    for batch in batches:
        # Tokenise & truncate the batch
        tokenised_batch = batch_tokenise_truncate(prompts=batch,
                                                  tokenizer=tokenizer,
                                                  max_input_tokens=max_input_tokens,
                                                  batch_size=len(batch))  # process entire batch
        ...
        # Prepare inputs for the model (move tensors to device)
        ...
        latencies.append((end_time - start_time) * 1000.0)  # milliseconds
        
        # Count generated tokens for each prompt in this batch.
