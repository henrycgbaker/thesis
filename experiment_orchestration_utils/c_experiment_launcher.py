...
def run_experiment_from_config(config_data, prompts, max_retries=3, retry_delay=5):
    """
...
def run_experiment_from_file(config_path, prompts):
    """
    Convenience function: load the config JSON from file and run the experiment.
    """
    with open(config_path, "r") as f:
        config_data = json.load(f)
    return run_experiment_from_config(config_data, prompts)
